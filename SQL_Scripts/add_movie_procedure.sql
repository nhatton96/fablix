...
BEGIN
  ...
  set sid = (select max(id) from stars where name = star limit 1);
  end if;
  
  if (select count(*) from stars_in_movies where starsId = sid and movieId = movie_id) = 0 then
  insert into stars_in_movies(starsId, movieId) values (sid, movie_id);
  end if;
END //
DELIMITER ;